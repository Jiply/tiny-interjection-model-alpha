...
The dataset contains {train_count} training rows and {validation_count} validation rows. Each row includes separate `prompt` and `completion` columns for completion-only tuning, plus the structured event stream, floor-control target, and combined `text` compatibility column.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Package validated TIM data for Hugging Face."
    )
