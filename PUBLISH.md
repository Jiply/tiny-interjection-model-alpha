...
- [ ] The public name is exactly **Tiny Interjection Model Alpha**; use **TIMA** only as its acronym.
- [ ] The repository, package metadata, image tags, job names, artifact names, API title, documentation, and license use the same current identity.
...
- [ ] No personal names, email addresses, usernames, home-directory paths, workstation details, account IDs, private repository names, or private URLs appear in tracked files or commit metadata.
...
- [ ] No cloud resource names, instance IDs, project IDs, subnet IDs, billing details, or other account identifiers are visible in text, screenshots, logs, or image metadata.
...
- [ ] Redact resource names, instance identifiers, account information, timestamps that reveal private operations, and any unrelated browser or desktop content.
...
- [ ] Reproduction commands use public resources or neutral placeholders pending explicit disclosure approval.
- [ ] Every placeholder is replaced with an approved public value before final challenge submission.
...
- [ ] Measured job status, timestamps, image digest, runtime, outputs, evaluation results, and billed cost replace planning estimates.
