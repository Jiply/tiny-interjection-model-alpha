...
def contains_expected_term(message: str, expected: str) -> bool:
    message_roots = {_word_root(word) for word in WORD_RE.findall(message.lower())}
    expected_roots = {_word_root(word) for word in WORD_RE.findall(expected.lower())}
    return bool(expected_roots) and expected_roots <= message_roots
...
def score_cases(engine: InteractionEngine, cases: list[BenchCase]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        joined_messages = " ".join(decision.target.messages)
        expected_contains = tuple(case.expected_contains)
        contains_ok = all(
            contains_expected_term(joined_messages, item) for item in expected_contains
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate TIM behavior.")
