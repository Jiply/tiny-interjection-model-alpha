...
from .format import format_training_text
...
def rows_from_examples(examples: Iterable[Example]) -> list[dict[str, Any]]:
    ...
    for example in examples:
        rows.append(
            example_to_dict(
                example, text=format_training_text(example.events, example.target)
            )
        )
    return rows
...
def build_dataset(
    *,
    ...
    train_count = write_jsonl(
        output_dir / "train.jsonl", rows_from_examples(train_examples)
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build TIMA training data.")
